...
/*   Updated: 2024/04/21 04:53:12 by picatrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ray->side == 0)
		ray->perpWallDist = ((double)ray->mapX - data.posX + \
				(1 - (double)ray->stepX) / 2) / ray->rayDirX;
	else
		ray->perpWallDist = ((double)ray->mapY - data.posY + \
				(1 - (double)ray->stepY) / 2) / ray->rayDirY;
	ray->h = WINDOW_HEIGHT;
